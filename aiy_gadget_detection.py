...
_COMPUTE_GRAPH_NAME  =  os.getcwd() + '/transfer100.binaryproto' #import frozen_inference_graph
...
def _decode_box_encoding(box_encoding, anchor):
    ...
    # Clamp value hingga range [0.0, 1.0], atau sebagian dari bounding box akan
    # jatuh di luar gambar
...
def _non_maximum_suppression(objs, overlap_threshold=0.5):
    ...
    for i in range(len(objs)):
        if objs[i].score < 0.0:
            continue
        # Menekan bounding box di sekitar yang nilainya lebih rendah dari boxes[i]
